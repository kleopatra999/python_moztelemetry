...
def test_empty_s3store(dummy_bucket):
...
def test_upload_key(dummy_bucket):
    store = S3Store(dummy_bucket.name)
    key = 'my-key'
    value = 'my-value'
    ...
    expected = dummy_bucket.Object(key).get()
    assert expected['Body'].read() == value


def test_list_keys(dummy_bucket):
    store = S3Store(dummy_bucket.name)
...
def test_list_folders(dummy_bucket):
    store = S3Store(dummy_bucket.name)
...
def test_get_key(dummy_bucket):
    ...
    with tempfile.TemporaryFile() as f:
        ...
        f.seek(0)
        store.upload_file(f, '', key)
    assert store.get_key(key) == value


def test_get_non_existing_key(dummy_bucket):
    store = S3Store(dummy_bucket.name)
...
def test_delete_key(dummy_bucket):
    store = S3Store(dummy_bucket.name)
    key = 'key1'
    value = 'value1'
...
def test_delete_non_existing_key(dummy_bucket):
    store = S3Store(dummy_bucket.name)
    store.delete_key('random-key-2')


def test_is_prefix_empty(dummy_bucket):
    store = S3Store(dummy_bucket.name)
    key = 'dir1/key1'
    value = 'value1'
